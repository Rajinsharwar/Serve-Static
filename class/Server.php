...
class Server {
	...
	public function missed_cron() {
		...
		return $message = sprintf(
			...
			$list,
			__( 'Please contact with your hosting provider to check if CRON is working.', 'serve-static' )
		);
	}
	...
	protected function get_events() {
		return [
			'serve_static_cache_cron_event'     => __( 'Scheduled Cache Regeneration', 'serve-static' ),
			'custom_warmup_cache_cron'          => __( 'Scheduled Event to get URLs for caching', 'serve-static' ),
			'warm_up_cache_request'             => __( 'Scheduled Event to regenerate cache', 'serve-static' ),
			'warm_up_cache_request_triggers'    => __( 'Scheduled Event to regenerate Cache for custom triggers', 'serve-static' ),
		];
	}
    ...
    public function cron_not_working(){
        ...
        ?>
        <div class="notice notice-error serve-static-notice is-dismissible">
            <?php echo wp_kses_post( $message, 'serve-static' ); ?>
            <p>
                <a class="serve-static-dismiss" href="<?php echo esc_url( add_query_arg( 'serve_static_dismiss_cron_fail_notice', 'true' ) ); ?>"><?php esc_html_e( 'Dismiss', 'serve-static' ); ?></a>
            </p>
        </div>
        <?php
    }
    ...
    public function cron_not_scheduled_notice(){
        ...
        <div class="notice notice-error serve-static-notice is-dismissible">
            <p>
                <?php esc_html_e( 'One of the scheduled cron events to regenerate the cache didn\'t get scheduled. This may happen due to some issues with your webserver or CRON system. Please ', 'serve-static' ); ?>
                <a href="<?php echo esc_url( admin_url('admin.php?page=serve_static_settings#stopwarming') ); ?>"><?php esc_html_e( 'Stop the regeneration' , 'serve-static' );?></a>
                <?php esc_html_e( ', and try again. If this continues, please post in our support forum, also confirm if your CRON is working properly.', 'serve-static' ); ?>
            </p>
                <a class="serve-static-dismiss" href="<?php echo esc_url( add_query_arg( 'serve_static_dismiss_cron_scheduled_notice', 'true' ) ); ?>"><?php esc_html_e( 'Dismiss', 'serve-static' ); ?></a>
            </p>
        </div>
        <?php
    }
